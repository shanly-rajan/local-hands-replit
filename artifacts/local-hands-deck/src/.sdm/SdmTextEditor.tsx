...
function updateParagraphElement(
  ...
  const effective = effectiveParagraph(body, paragraph);
  const defaultStyle =
    node.content.size === 0
  ...
  if (paragraph.bullet !== undefined && paragraph.bullet.kind !== 'none') {
...
function paragraphNodeView(
  ...
  updateParagraphElement(dom, node, body, theme);
  ...
  return {
    ...
    update(nextNode: ProseMirrorNode) {
      ...
      updateParagraphElement(dom, nextNode, body, theme);
...
export function SdmTextEditor({
  ...
  useEffect(() => {
    ...
    const view: EditorView = new EditorView(mount, {
      ...
      nodeViews: {
        paragraph: (node) =>
          paragraphNodeView(node, bodyRef.current, theme),
      },
      handleDOMEvents: {
