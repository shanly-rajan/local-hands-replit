...
export function canonicalizeTextBody(body: TextBody): TextBody {
  ...
  const next: TextBody = {
    ...
    ...(body.listStyle === undefined ? {} : { listStyle: body.listStyle }),
...
export function textBodyToDoc(body: TextBody): ProseMirrorNode {
  ...
  const paragraphs =
    canonical.paragraphs.length === 0
      ...
      : canonical.paragraphs.map((paragraph) =>
          paragraphNode(paragraph, canonical.listStyle),
        );

  return sdmTextSchema.nodes.doc.create(
    {
      insetsPt: canonical.insetsPt ?? null,
      listStyle: canonical.listStyle ?? null,
...
export function listStyleFromPmAttrs(
  attrs: Record<string, unknown>,
): ListStyle | undefined {
  return Value.Check(ListStyleSchema, attrs.listStyle)
    ? attrs.listStyle
    : undefined;
}
...
export function defaultRunStyleAttr(
  doc: ProseMirrorNode,
  paragraphAttrs: Record<string, unknown>,
  style: RunStyle,
): RunStyle | null {
  const paragraph = paragraphFromPmAttrs(paragraphAttrs);
  const inherited = listLevelStyle(
    listStyleFromPmAttrs(doc.attrs),
    paragraph.level ?? 0,
  )?.defaultRunStyle;
  const overrides = runStyleOverrides(style, inherited);
...
function paragraphFromNode(
  ...
  const effective = effectiveParagraphWithListStyle(listStyle, overrides);
...
export function docToTextBody(doc: ProseMirrorNode): TextBody {
  ...
  const listStyle = listStyleFromPmAttrs(attrs);
  const paragraphs: Array<Paragraph> = [];
  doc.forEach((node) => {
    if (node.type !== sdmTextSchema.nodes.paragraph) {
      return;
    }
    const paragraph = paragraphFromNode(node, listStyle);
