...
function defaultStyleOf(doc: ProseMirrorNode, node: ProseMirrorNode): RunStyle {
  const paragraph = paragraphFromPmAttrs(node.attrs);

  return effectiveParagraphWithListStyle(
    listStyleFromPmAttrs(doc.attrs),
    paragraph,
  ).defaultRunStyle;
}

function styleAtEnd(doc: ProseMirrorNode, node: ProseMirrorNode): RunStyle {
  if (node.childCount === 0) {
    return defaultStyleOf(doc, node);
  }
  const child = node.child(node.childCount - 1);

  return canonicalizeRunStyle({
    ...defaultStyleOf(doc, node),
...
function styleFromSelectionHead(state: EditorState): RunStyle {
  ...
  if (parent.type !== sdmTextSchema.nodes.paragraph) {
    ...
    return last === undefined ? {} : styleAtEnd(state.doc, last.node);
  }
  const marks = state.storedMarks ?? state.selection.$head.marks();

  return canonicalizeRunStyle({
    ...defaultStyleOf(state.doc, parent),
...
function seedTransaction(state: EditorState): Transaction | undefined {
  ...
  const style = defaultStyleOf(state.doc, parent);
...
export const formattingContinuityPlugin = new Plugin({
  ...
  props: {
    handleTextInput(view, from, to, text) {
      ...
      const style = defaultStyleOf(view.state.doc, parent);
    ...
    if (transactions.some((candidate) => candidate.docChanged)) {
      const selectionParagraphPosition =
        newState.selection.$from.parent.type === sdmTextSchema.nodes.paragraph
          ...
          : undefined;
      for (const { current, previous } of mappedParagraphs(
        transactions,
        oldState,
        newState,
      )) {
        ...
        const style = canonicalizeRunStyle({
          ...defaultStyleOf(newState.doc, current.node),
          ...(styleFromOldSelection(oldState, previous.position) ??
            styleAtEnd(oldState.doc, previous.node)),
        });
        if (Object.keys(style).length === 0) {
          continue;
        }
